    ...
    filteredProjects.map(async (project) => {
      ...
      return {
        ...
        fieldData: {
          ...
          status: statusLabel,
          'project-type': project.fieldData['project-type'],
        },
      }
    })
  )
...
/**
 * Get all published projects with full details (bypasses cache).
 * Useful for debugging and admin endpoints.
 * @returns An array of all published (non-draft, non-archived) projects with full field data.
 */
export const getAllPublishedProjectsFull = async (): Promise<Project[]> => {
  // Fetch projects directly from Webflow (bypassing cache)
  const projects = await listCollectionItems<Project>(COLLECTION_ID_PROJECTS)
  
  // Filter to only published projects
  return projects.filter((project) => !project.isDraft && !project.isArchived)
}
